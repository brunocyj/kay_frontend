...
export default function LoginModal() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : t.login_error_default);
    } finally {
      setLoading(false);
    }
  }
    ...
    <>
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 pointer-events-none">
        ...
        >
          ...
          <div className="mb-7">
            <h2 className="text-lg font-semibold text-gray-900">{t.login_title}</h2>
            <p className="text-sm text-gray-400 mt-0.5">{t.login_subtitle}</p>
          </div>

          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            <div className="flex flex-col gap-1.5">
              <label className="text-xs font-medium text-gray-600">{t.login_label_id}</label>
              <input
                ref={inputRef}
                type="text"
                value={identifier}
            ...
            <div className="flex flex-col gap-1.5">
              <label className="text-xs font-medium text-gray-600">{t.login_label_pw}</label>
              <input
                type="password"
                value={password}
